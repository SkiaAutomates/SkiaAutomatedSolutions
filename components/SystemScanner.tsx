...
import './SystemScanner.css'; // New CSS file
...
class CardStreamController {
    ...
    constructor(container: HTMLElement, cardLine: HTMLElement, speedIndicator: HTMLElement, scannerCallback: (active: boolean) => void) {
    ...
    setupEventListeners() {
        this.cardLine.addEventListener("mousedown", this.onStartDragHandler);
        document.addEventListener("mousemove", this.onDragHandler); // Bind to document for global drag
        document.addEventListener("mouseup", this.onEndDragHandler); // Bind to document for global release

        // Touch events
        this.cardLine.addEventListener("touchstart", this.onTouchStartHandler, { passive: false });
        document.addEventListener("touchmove", this.onTouchMoveHandler, { passive: false });
        document.addEventListener("touchend", this.onEndDragHandler);

        this.cardLine.addEventListener("wheel", this.onWheelHandler);
        this.cardLine.addEventListener("selectstart", (e) => e.preventDefault());
        this.cardLine.addEventListener("dragstart", (e) => e.preventDefault());

        this.onResize = this.calculateDimensions.bind(this);
        window.addEventListener("resize", this.onResize);
    }

    onStartDragHandler = (e: MouseEvent) => this.startDrag(e);
    onDragHandler = (e: MouseEvent) => this.onDrag(e);
    onEndDragHandler = () => this.endDrag();
    onTouchStartHandler = (e: TouchEvent) => this.startDrag(e.touches[0] as unknown as MouseEvent);
    onTouchMoveHandler = (e: TouchEvent) => this.onDrag(e.touches[0] as unknown as MouseEvent);
    onWheelHandler = (e: WheelEvent) => this.onWheel(e);
    onResize: () => void = () => { };

    startDrag(e: MouseEvent) {
        // e.preventDefault(); // Sometimes prevents clicking, but needed for drag
        ...
        const style = window.getComputedStyle(this.cardLine);
    ...
    endDrag() {
        ...
        if (Math.abs(this.mouseVelocity) > this.minVelocity) {
            ...
            // Keep momentum or reset to default
            // this.velocity = 120; 
        }
    ...
    toggleAnimation() {
        ...
        return this.isAnimating; // Return new state
    }
    ...
    onWheel(e: WheelEvent) {
        ...
        const delta = e.deltaY > 0 ? -scrollSpeed : scrollSpeed; // Invert for natural feel
    ...
    // ... Helper methods (generateCode, etc.) ...
    ...
    updateCardClipping() {
        ...
        this.scannerCallback(anyScanningActive);
    }

    updateAsciiContent() {
        const asciiContents = this.cardLine.querySelectorAll(".ascii-content");
    ...
    populateCardLine() {
        ...
        const cardsCount = 30; // Provided value
    ...
    destroy() {
        ...
        this.cardLine.removeEventListener("mousedown", this.onStartDragHandler);
        document.removeEventListener("mousemove", this.onDragHandler);
        document.removeEventListener("mouseup", this.onEndDragHandler);

        this.cardLine.removeEventListener("touchstart", this.onTouchStartHandler);
        document.removeEventListener("touchmove", this.onTouchMoveHandler);
        document.removeEventListener("touchend", this.onEndDragHandler);

        this.cardLine.removeEventListener("wheel", this.onWheelHandler);
    }
}
...
class ParticleSystem {
    ...
    constructor(canvas: HTMLCanvasElement) {
        ...
        // this.camera.position.z = 100; // done in init
        // this.renderer // done in init
...
class ParticleScanner {
    ...
    initParticles() {
        for (let i = 0; i < this.maxParticles; i++) {
            const particle: any = this.createParticle();
    ...
    drawLightBar() {
        ...
        // G1
        const g1 = this.ctx.createLinearGradient(this.lightBarX - lineWidth * 2, 0, this.lightBarX + lineWidth * 2, 0);
        g1.addColorStop(0, "rgba(139, 92, 246, 0)"); g1.addColorStop(0.5, `rgba(196, 181, 253, ${0.8 * glowIntensity})`); g1.addColorStop(1, "rgba(139, 92, 246, 0)");
        this.ctx.globalAlpha = glow1Alpha; this.ctx.fillStyle = g1;
        this.ctx.beginPath(); this.ctx.roundRect(this.lightBarX - lineWidth * 2, 0, lineWidth * 4, this.h, 25); this.ctx.fill();

        // G2
        const g2 = this.ctx.createLinearGradient(this.lightBarX - lineWidth * 4, 0, this.lightBarX + lineWidth * 4, 0);
        g2.addColorStop(0, "rgba(139, 92, 246, 0)"); g2.addColorStop(0.5, `rgba(139, 92, 246, ${0.4 * glowIntensity})`); g2.addColorStop(1, "rgba(139, 92, 246, 0)");
        this.ctx.globalAlpha = glow2Alpha; this.ctx.fillStyle = g2;
        this.ctx.beginPath(); this.ctx.roundRect(this.lightBarX - lineWidth * 4, 0, lineWidth * 8, this.h, 35); this.ctx.fill();

        // G3
        if (this.scanningActive) {
            const g3 = this.ctx.createLinearGradient(this.lightBarX - lineWidth * 8, 0, this.lightBarX + lineWidth * 8, 0);
            g3.addColorStop(0, "rgba(139, 92, 246, 0)"); g3.addColorStop(0.5, "rgba(139, 92, 246, 0.2)"); g3.addColorStop(1, "rgba(139, 92, 246, 0)");
            this.ctx.globalAlpha = glow3Alpha; this.ctx.fillStyle = g3;
    ...
    render() {
        ...
        if (Math.random() < this.intensity && this.count < this.maxParticles) {
            const p: any = this.createParticle(); p.originalAlpha = p.alpha; p.startX = p.x; this.count++; this.particles[this.count] = p;
        }
        // Burst spawn based on intensity
        if (intensityRatio > 1.1 && Math.random() < (intensityRatio - 1.0) * 1.2) { const p: any = this.createParticle(); p.originalAlpha = p.alpha; p.startX = p.x; this.count++; this.particles[this.count] = p; }
        if (intensityRatio > 1.3 && Math.random() < (intensityRatio - 1.3) * 1.4) { const p: any = this.createParticle(); p.originalAlpha = p.alpha; p.startX = p.x; this.count++; this.particles[this.count] = p; }
...
export const SystemScanner: React.FC = () => {
    ...
    const [controller, setController] = useState<CardStreamController | null>(null);

    useEffect(() => {
        if (!containerRef.current || !cardLineRef.current || !particleCanvasRef.current || !scannerCanvasRef.current || !speedIndicatorRef.current) return;
        ...
        const cardController = new CardStreamController(
            containerRef.current,
            cardLineRef.current,
            speedIndicatorRef.current,
            (active) => scannerVisuals.setScanningActive(active)
        );
    ...
    // Removed unused toggle/reset/change handlers since UI is hidden

    return (
        <section className="relative system-scanner-wrapper py-24">

            <div className="relative z-20 text-center mb-16 px-4 pointer-events-none">
                <h2 className="text-4xl md:text-6xl font-black text-white mb-6 uppercase tracking-tight font-display drop-shadow-2xl">
                    Stop Burning Cash on <br className="hidden md:block" />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500 text-glow">Chaotic Systems</span>
                </h2>
                <p className="text-lg md:text-xl text-blue-200/80 max-w-3xl mx-auto font-mono text-sm tracking-widest mt-6 bg-black/40 backdrop-blur-sm p-4 rounded-xl border border-white/5 inline-block">
                    WE SCAN YOUR BUSINESS AND REVEAL THE HIDDEN CODE TO <span className="text-cyan-400 font-bold">SCALABLE WEB APPS</span> AND <span className="text-purple-400 font-bold">AUTOMATION</span>
                </p>
            </div>

            <div className="container relative z-10" ref={containerRef} style={{ height: '400px' }}>
                <canvas id="particleCanvas" ref={particleCanvasRef} className="absolute inset-0 pointer-events-none"></canvas>
                <canvas id="scannerCanvas" ref={scannerCanvasRef} className="absolute inset-0 pointer-events-none"></canvas>

                <div className="scanner pointer-events-none"></div>
            ...
            {/* Hidden speed indicator for logic */}
            <div className="hidden">
                <span ref={speedIndicatorRef}>120</span>
            </div>
        </section>
    );
};
